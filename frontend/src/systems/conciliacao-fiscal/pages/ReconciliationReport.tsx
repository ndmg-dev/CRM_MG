...
export default function ReconciliationReport() {
  const { companyId: empresa_id, periodo } = useParams<{ companyId: string; periodo: string }>()
  ...
  useEffect(() => {
    setLoading(true)
    Promise.all([
      apiFetch(`/conciliacoes/${empresa_id}/${periodo}/relatorio?limit=1000&status=${statusFilter}`).then((res) => {
        if (!res.ok) throw new Error(`Erro ${res.status}`)
        return res.json()
      }),
      apiFetch(`/conciliacoes/${empresa_id}/historico`).then((res) => {
        if (!res.ok) throw new Error(`Erro ${res.status}`)
        return res.json()
      }),
      ...
      <div className="flex justify-between items-start">
        ...
        <div className="flex flex-col gap-3 items-end">
          <div className="flex gap-3">
            ...
            <button
              onClick={() => downloadFile(`/conciliacoes/${empresa_id}/${periodo}/exportar.csv`, `conciliacao_${periodo}.csv`)}
            ...
            <button
              onClick={() => downloadFile(`/conciliacoes/${empresa_id}/${periodo}/exportar.xlsx`, `conciliacao_${periodo}.xlsx`)}
              className="btn-ghost px-4 py-2 rounded-lg text-sm inline-flex items-center gap-2"
            >
              <Download size={14} />
              Excel
