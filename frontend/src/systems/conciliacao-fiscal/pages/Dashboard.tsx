...
import { Briefcase, FileText, Database, ArrowRight, Activity } from 'react-feather'
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex items-end justify-between">
        ...
        <Link to={toAbs('companies/new')} className="btn-gold px-5 py-2.5 rounded-xl text-sm font-semibold inline-flex items-center gap-2">
          <Briefcase size={16} />
          Nova Empresa
        </Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {data.recent_reconciliations.length === 0 ? (
            ...
            <div className="space-y-3">
              {data.recent_reconciliations.map((r, i) => (
                <Link
                  key={i}
                  to={toAbs(`reconciliations/${r.empresa_id}/${r.periodo}`)}
        ...
        <div className="glass-card rounded-2xl p-6">
          <div className="flex items-center justify-between mb-5">
            ...
            <Link to={toAbs('companies')} className="text-xs text-[var(--gold)] hover:text-[var(--gold-light)] inline-flex items-center gap-1 transition-colors">
          ...
          {data.empresas.length === 0 ? (
            <div className="text-center py-8">
              ...
              <Link to={toAbs('companies/new')} className="btn-gold px-4 py-2 rounded-lg text-sm font-semibold">
                Cadastrar Empresa
              </Link>
            </div>
          ) : (
