...
export default function CompanyDetails() {
  ...
  const fetchXmlsForMonth = async (mes: string | null) => {
    ...
    try {
      const url = mes
        ? `/empresas/${id}/xml/list?mes=${mes}`
        : `/empresas/${id}/xml/list`
  ...
  useEffect(() => {
    ...
    apiFetch(`/conciliacoes/${id}/historico`)
      .then((res) => {
        if (!res.ok) throw new Error(`Erro ${res.status}`)
        return res.json()
      })
    ...
    apiFetch(`/empresas/${id}/xml/summary`)
      .then((res) => {
        if (!res.ok) throw new Error(`Erro ${res.status}`)
        return res.json()
      })
  ...
  async function handleAutomatedFlow(e: React.FormEvent) {
    ...
    try {
      const spedRes = await apiFetch(`/empresas/${id}/sped/upload`, {
        method: 'POST',
        body: formDataSped,
      })
      ...
      const reconRes = await apiFetch(
        `/conciliacoes?empresa_id=${id}&periodo=${periodo}&sync_sieg=${syncSieg}`,
        { method: 'POST' },
      )
      ...
      if (data.warning) {
        navigate(toAbs(`reconciliations/${id}/${periodo}?warning=${encodeURIComponent(data.warning)}`))
      } else {
        navigate(toAbs(`reconciliations/${id}/${periodo}`))
      }
    } catch (err: any) {
      setLogs((prev) => {
        const newLogs = [...prev]
  ...
  async function handleAudit(e: React.FormEvent) {
    ...
    try {
      const res = await apiFetch(`/conciliacoes/${id}/auditoria/xml?mes=${auditPeriod}`)
                ...
                {history.map((h, idx) => (
                  <div key={idx} className="glass-card rounded-xl p-6 relative group border border-white/5 hover:border-[var(--gold-border)] transition-all flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <Link to={toAbs(`reconciliations/${id}/${h.periodo}`)} className="text-xs font-bold text-[var(--gold)] bg-[var(--gold-glow)] px-3 py-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap">
                        Ver Relatório
                      </Link>
                    </div>
            ...
            <div className="space-y-6">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const xmlRes = await apiFetch(`/empresas/${id}/xml/upload`, {
                      method: 'POST',
                      body: formDataXml,
                    })
                    if (!xmlRes.ok) {
