        ...
        <RSelect.Content className={styles.content} position="popper" sideOffset={6}>
          ...
          <RSelect.Viewport className={styles.viewport}>
            {options.map((opt) => (
              ...
              >
                <RSelect.ItemText>
                  {/* RSelect.ItemText não repassa `className` para o span que renderiza
                      (ele só usa `id`/children — testado via getComputedStyle, a classe
                      nunca chega ao DOM) — por isso o truncamento vai num span interno. */}
